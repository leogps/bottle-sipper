...
def wrap_in_column(content, style_class):
    html = Template(TABLE_COL_TEMPLATE)
    return html.substitute(table_col=content, style_class=style_class)


def wrap_in_table(content):
    html = Template(TABLE_TEMPLATE)
    return html.substitute(table_content=content)


def wrap_in_row(content):
    html = Template(TABLE_ROW_TEMPLATE)
    return html.substitute(table_row=content)
...
class Sipper(Thread):
    ...
    def serve(self, url_path=''):
        ...
        mime_type = None
        if ext in MIME_TYPE_EXTENSION_MAP:
            mime_type = MIME_TYPE_EXTENSION_MAP[ext]
    ...
    def handle_dir(self, path):
        ...
        dir_list = sorted(os.listdir(path), key=lambda v: v.upper())
        ...
        for f in dir_list:
            ...
            modified_date_td = wrap_in_column(modified_date, '')

            stat_info = os.stat(file_path)
            if os.path.isfile(file_path):
                size = stat_info.st_size
                size_formatted = sizeof_fmt(size)
                size_td = wrap_in_column(size_formatted, 'file-size')
            else:
            ...
            columns.append(modified_date_td)
            columns.append(size_td)
            columns.append(link_td)

            row = wrap_in_row(''.join(columns))
            rows.append(row)
...
TABLE_COL_TEMPLATE = """
    <td class="$style_class">$table_col</td>
"""
